...
- **2026-04-25** — Matrix dispatcher deployed: PM2 daemon (`matrix-dispatcher`, id 31) watches 6 agent Matrix rooms for messages from `@ted`, spawns `claude -p` sessions, and posts responses back with acknowledgment, @mention, and response chunking. Six per-agent Synapse accounts (`@agent.research`, `@agent.dev`, etc.) created with scoped-mcp manifests for mid-session posting. Phase 1 (spawn-only) live and validated; v2 (SQLite resume), v3 (commands), and Phase 4 (hardening) planned. 3 Low security findings resolved post-deployment. See [matrix-dispatcher](docs/components/matrix-dispatcher.md).
