...
- **2026-04-21** — New public showcase repo: [ollama-queue-proxy](https://github.com/TadMSTR/ollama-queue-proxy) — drop-in HTTP proxy for Ollama with three-tier priority queuing (high/normal/low), per-client API key auth with priority ceilings, model-aware failover across multiple Ollama hosts, and a Prometheus metrics endpoint. Auth-first design: keys are scoped, management endpoints are gated, and webhook SSRF protection covers both IP literals and hostnames.
