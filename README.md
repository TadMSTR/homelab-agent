...
- **2026-04-08** — jobsearch-mcp v2: Ollama bge-m3 embeddings replace Voyage AI, Valkey enrichment cache, three-tier JD pipeline (Firecrawl→Crawl4AI→rawFetch), five new resume profile tools (`build_profile`/`save_profile`/`tailor_resume`/get/delete), ATS scoring, job-watcher email alerts, USAJobs as default source. Stack 3→5 containers. Also: pm2-mcp (structured PM2 CLI access for agents), ntfy-mcp (native push notification tool), searxng-mcp v3.1.0 (recency weighting), and Grafana/Backrest MCP servers converted to HTTP mode via SuperGateway for Claude Desktop and Claude Code.
- **2026-04-06** — GPU-accelerated semantic memory: memsearch v0.2.x switches to Milvus standalone, bge-m3 embeddings via the forge GPU (1024-dim, 8192-token context), and a cross-encoder reranker. Files that previously failed to index under the 512-token nomic-embed-text limit all index cleanly now. Same bge-m3 model now shared between memsearch and Graphiti — Voyage AI dependency dropped entirely.
...
- **2026-04-02** — doc-sync documentation cache (42+ services fetched nightly, memsearch-indexed), helm-ops-mcp SSH remote host MCP server, librarian-weekly prime-directive sync, and repo-sync-nightly auto-commit policy for doc repos.
- **2026-03-29** — Inter-agent event bus (NATS JetStream federation, reconciler, JSONL audit trail), Temporal durable workflow engine for multi-phase Helm builds, tiered memory pipeline (daily Haiku promotion + weekly Opus distillation), and real-time memsearch-watch indexing.
