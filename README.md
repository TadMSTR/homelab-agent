...
- **2026-04-26** — Memory lifecycle overhaul across 9 phases: added `category:` frontmatter to 2818 memory notes with six lifecycle policies (two expiring: `transient-finding` 90d, `session-summary` 30d; four durable-never-expire). Deployed OpenSearch 2.19.1 as a full-text search backend alongside a SQLite WAL-mode metadata index (2819 notes). Built two FastMCP servers: **memory-metadata-mcp** (port 8490, all agents — structured filtered queries via `list_notes`/`count_by`) and **memory-search-mcp** (port 8491, personal-agent only — full-text body search). Added **memory-os-sync** PM2 daemon (30s batch sync to OpenSearch) and **memory-archive-mirror** cron (02:30 daily NFS rsync to atlas). Seeded `design-records` Gitea repo for permanent durable note retention. SearXNG and LibreChat configs are untouched — memory excerpts never surface in shared search. OpenSearch isolated to dedicated `memory-search-net` (removed from `claudebox-net`). See [memory-lifecycle](docs/components/memory-lifecycle.md).

- **2026-04-25** — Matrix dispatcher v0.1–v0.4 deployed in a single day: PM2 daemon (`matrix-dispatcher`, id 31) watches 6 agent Matrix rooms for messages from `@ted`, spawns `claude -p` sessions, and posts responses back. v0.2 added SQLite session persistence with thread-based resume (plus an `event_aliases` table to handle Element's `m.in_reply_to` reply quirk). v0.3 added bang-prefix commands (`!sessions`, `!recap`, `!mirror`, `!cancel`, `!help`) — `!` instead of `/` because Element intercepts `/`-prefixed messages client-side as IRC commands. v0.4 rewrote the subprocess layer as async, added per-room concurrency locks, per-room rate limits, `!cancel`, and a startup notification. Six per-agent Synapse accounts with scoped-mcp manifests for mid-session posting. 8 Low security findings resolved across phases. See [matrix-dispatcher](docs/components/matrix-dispatcher.md).
- **2026-04-23** — Matrix agent communications stack deployed: Synapse v1.151.0 + PostgreSQL 16 + Element Web v1.12.15 as the primary agent notification layer; [`matrix-mcp`](https://github.com/TadMSTR/matrix-mcp) FastMCP server (port 8487) gives all agents `send_matrix_message`, `post_artifact`, `get_matrix_messages`, and `list_matrix_rooms` tools; [`matrix-channel`](https://github.com/TadMSTR/matrix-channel) Node.js Channel plugin enables permission relay from Element Web into active Claude Code sessions. ntfy retained for dead-letter and pending-approval only. 7 security findings resolved. See [matrix](docs/components/matrix.md).
