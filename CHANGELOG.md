...
**matrix-dispatcher** — Deployed `matrix-dispatcher` (PM2 id 31), a daemon that polls 6 agent Matrix rooms every 5s for messages from `@ted`, spawns `claude -p` sessions with the agent's project directory as cwd, and posts responses back. Phase 1 features: immediate acknowledgment message (`Working... (session <uuid>)`), `@ted:yourdomain` mention on all responses for Element push notifications, and paragraph-boundary response chunking for stdout exceeding 4000 chars. Six per-agent Synapse accounts (`@agent.{research,claudebox,dev,homelab-ops,security,writer}:yourdomain`) created and joined to their primary rooms plus `#approvals` and `#announcements`; scoped-mcp manifests written to `~/.claude/manifests/`. Dispatcher credentials stored at `~/.claude-secrets/matrix-dispatcher.env` (chmod 600), sourced at startup via `start.sh` wrapper. v2 (SQLite + thread-based resume), v3 (`/sessions`, `/recap`, `/mirror`, `/help`, nightly cleanup), and Phase 4 (concurrency, timeout, rate limiting, `/cancel`) are planned next phases.

**Security hardening (3 Low findings resolved):** L1 — poll-token file write made atomic (`.tmp` + rename) so daemon restarts during a write don't corrupt the sync state; L2 — `CLAUDE_*` glob removed from subprocess env allowlist, replaced with an explicit allowlist of required vars (dispatcher credentials no longer flow into agent processes); L3 — confirmed parameterized SQLite queries throughout (no f-string SQL construction).
