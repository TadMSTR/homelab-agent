...
**Security hardening (4 findings resolved):** M1 (medium) OpenSearch unauthenticated lateral access — moved opensearch off the shared agent network onto a dedicated single-member `memory-search-net`; 35+ containers on the shared network can no longer reach the `claude-memory` index (verified: DNS resolution failure from searxng post-move); `memory-search-mcp` and `memory-os-sync` continue reaching OpenSearch via host loopback `127.0.0.1:9200`. L1 OpenSearch image SHA-pinned to digest `sha256:72fe2fc8…e89a4` (tag-only was claimed as pinned in build plan). L2 NFS archive directories hardened from 0755 to 0700; `umask 0077` added to `memory-archive-mirror.sh` for new subdirectory creation. L3 Docker Compose header comment corrected — false SWAG/Authelia claim removed, accurate loopback-only exposure documented.

---

## 2026-04-25

**matrix-dispatcher v0.2 + v0.3 + v0.4** — All planned dispatcher phases shipped same day. **v0.2** added SQLite session persistence (`~/.claude/data/matrix-dispatcher/sessions.db`, WAL mode, parameterized queries throughout) with three tables: `sessions` (thread_root_id → session_id mapping), `event_aliases` (maps ack and chunk event IDs back to sessions — handles Element's `m.in_reply_to` reply format), and `poll_state` (replaces v1 `poll-tokens.json`, one-time migration on startup). Thread replies now resume via `claude -p --resume <session_id>`; sessions survive PM2 restarts. **v0.3** added bang-prefix commands intercepted before any spawn: `!sessions` (10 most recent in room, reply to resume), `!recap [N]` (last N turns, default 5, read-only), `!mirror` (register a CloudCLI-started JSONL session under a new thread root for resume), `!help`. The `!` prefix was chosen because Element intercepts `/`-prefixed messages client-side as IRC commands and never sends them to Matrix. 30-day retention cleanup runs at startup and every 24h. **v0.4** rewrote the subprocess layer to `asyncio.create_subprocess_exec` + `asyncio.wait_for` (resolves v0.2 audit L2 — sync `subprocess.run` had blocked the entire poll loop), added per-room concurrency lock (`_room_locks`), per-room spawn rate limit (10s minimum gap), `!cancel` (SIGTERM the active subprocess in the room), and a startup notification posted to a configurable room (default `#claudebox`). Active processes tracked in `_active_processes[room_id]` for cancel routing.

**Security hardening across phases (8 Low findings resolved total):** v0.1 L1/L3 (atomic poll-token write), v0.1 L2 (`CLAUDE_*` glob removed from subprocess env allowlist), v0.2 L1 (`sessions.db` chmod 600 at creation), v0.2 L3 (`extract_thread_root` type guard), v0.4 L1 (concurrent task handlers with paired race fixes — cancel registration wait, rate-limit and `_post_response` inside the lock, clean shutdown drain), v0.4 L2 (`communicate()` drain after kill prevents pipe leak), v0.4 L3 (`uuid.UUID()` parse check on JSONL stems before passing to `--resume` argv). v0.2 L2 (sync subprocess in async loop) was deferred to Phase 4 per build plan scope and resolved by the v0.4 async rewrite.

**matrix-dispatcher v0.1** — Deployed `matrix-dispatcher` (PM2 id 31), a daemon that polls 6 agent Matrix rooms every 5s for messages from `@ted`, spawns `claude -p` sessions with the agent's project directory as cwd, and posts responses back. Phase 1 features: immediate acknowledgment message (`Working... (session <uuid>)`), `@ted:yourdomain` mention on all responses for Element push notifications, and paragraph-boundary response chunking for stdout exceeding 4000 chars. Six per-agent Synapse accounts (`@agent.{research,claudebox,dev,homelab-ops,security,writer}:yourdomain`) created and joined to their primary rooms plus `#approvals` and `#announcements`; scoped-mcp manifests written to `~/.claude/manifests/`. Dispatcher credentials stored at `~/.claude-secrets/matrix-dispatcher.env` (chmod 600), sourced at startup via `start.sh` wrapper. v2 (SQLite + thread-based resume), v3 (`/sessions`, `/recap`, `/mirror`, `/help`, nightly cleanup), and Phase 4 (concurrency, timeout, rate limiting, `/cancel`) are planned next phases.

**matrix-management-bot** — Deployed `matrix-admin-bot` ([`TadMSTR/matrix-admin-bot`](https://github.com/TadMSTR/matrix-admin-bot)), a PM2-managed Python asyncio bot running as `@matrix-admin-bot:yourdomain` on the claudebox Synapse homeserver. The bot provisions and manages agent Matrix accounts via 8 commands gated by an `allowed_senders` allowlist: `!create-account`, `!join-room`, `!list-rooms`, `!audit-memberships`, `!revoke-token`, `!restore-token`, `!rotate-token`, `!status`. Token rotation is SIGTERM-safe — in-flight command tasks are tracked in `_command_tasks` and skipped by the signal handler, giving multi-step commands until PM2's `kill_timeout` (5000 ms) to finish. New tokens are written atomically to `scoped-mcp-<agent>.yml` using `.tmp` + `os.replace()`. 39 tests passing.

**Security hardening (4 Low findings resolved):** `!join-room` requires the full `!room_id:yourdomain` form and validates the room against the target agent's `allowed_rooms` — no substring resolution or literal-room-id bypass (L1); `room_id` validated with `_ROOM_ID_RE` regex before URL interpolation, mirroring the MXID validation pattern (L2); both `config.save_device_id()` and `synapse.write_token_file()` use `.tmp` + `os.replace()` — secrets file uses `os.open(..., 0o600)` so mode survives the rename (L3); `_handle_signal` tracks command coroutines in a separate set and skips them when cancelling (L4). Commits: `060bdd1` (L2/L3/L4), `122cf55` (L1).
...
- **11 agent rooms** — `#task-queue`, `#approvals`, `#research`, `#claudebox`, `#dev`, `#helm-build`, `#homelab-ops`, `#security`, `#outreach`, `#writer`, `#announcements`. Bot user `@claude-agent:<your-server-name>` joined to all rooms.
