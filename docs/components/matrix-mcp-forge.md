...
via the `your-forge-domain` homeserver. It is a separate PM2 process from the claudebox
`matrix-mcp` instance — different homeserver, different bot account, different port.

See [matrix.md](matrix.md) for the claudebox matrix-mcp. This doc covers the forge
deployment only.

- **Source:** `~/repos/personal/matrix-mcp/`
- **PM2 name:** `matrix-mcp-forge` (ID 20)
...
- [matrix.md](matrix.md) — claudebox matrix-mcp (port 8487 on claudebox)
