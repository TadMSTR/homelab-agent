...
`your-forge-domain`. It provides the Matrix communications backbone for forge's operator
agents — the same role that claudebox's Synapse plays for claudebox agents, but scoped to
the forge environment.

See [matrix.md](matrix.md) for the claudebox homeserver. This doc covers the forge
deployment only.
...
clients can locate `matrix.your-forge-domain` when connecting as `your-forge-domain`. This
...
Key settings:
- `server_name: your-forge-domain` — the Matrix domain, not the actual hostname
...
Ketesa is a web-based Synapse admin UI accessible at `ketesa.your-forge-domain`, gated
...
- `@forge-sysadmin:your-forge-domain` — matrix-dispatcher bot (reads rooms, routes to agents)
- `@matrix-admin-bot:your-forge-domain` — admin bot (account provisioning)
- Agent MXIDs follow the pattern `@agent.<type>:your-forge-domain`
...
- [matrix.md](matrix.md) — claudebox homeserver (separate deployment)
